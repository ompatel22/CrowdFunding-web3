...
/**
 * Calculates the remaining days until the campaign deadline.
 * @param {number} deadline - The campaign deadline in Unix timestamp (seconds).
 * @returns {number} Remaining days until the deadline.
 */
...
const CampaignDetails = () => {
  ...
  const remainingDays = daysLeft(state.deadline); // Fix applied here
  ...
  const fetchDonators = async () => {
    ...
    const updatedCampaign = updatedCampaigns[state.pId];

    if (updatedCampaign) {
      state.amountCollected = updatedCampaign.amountCollected.toString();
    }
  };
    ...
    <div>
      ...
      <div className="w-full flex md:flex-row flex-col mt-10 gap-[30px]">
        ...
        <div className="flex md:w-[150px] w-full flex-wrap justify-between gap-[30px]">
          ...
          <CountBox
            title={`Raised of ${(Number(state.target) / 1e18).toFixed(6)} ETH`}
            value={`${(Number(state.amountCollected) / 1e18).toFixed(6)} ETH`}
          />
